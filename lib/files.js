...
module.exports = function(app) {
    app.when(function *() {
        ...
        if(cmd === "build" || cmd === "rebuild") {
            ...
            // calc simhash
            files = yield files.map(coSimhash);
            ...
            files.forEach(function(file) {
                if(!cachedFilesObj[file.path]) {
                    file.createTime = now;
                    file.modifiedTime = now;
                    newFiles.push(file);
                    return;
                }
                if(cachedFilesObj[file.path].sha1 === file.sha1) {
                    unchangedFiles.push(cachedFilesObj[file.path]);
                } else {
                    file.modifiedTime = now;
            ...
            newFiles = newFiles.map(function(file) {
                removedFiles.forEach(function(removedFile) {
                    if(hammingDistance(file.simhash, removedFile.simhash) < 4) {
                        console.log("Rename Detected: " + removedFile.path + " -> " + file.path);
                        console.log("Set file.createTime to: " + removedFile.createTime);
